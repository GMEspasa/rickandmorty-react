...
const CharacterSpecs = () => {
  ...
  const { characterSelected, setCharacterSelected,  stateData} = useContext(StateContext);
  const [charFilterId] = stateData.filter(element => element.id === characterSelected)

  setCharacterSelected(characterSelected);

  return (
    <div className="w-1/4 character-specs-container">
      <Character name={charFilterId.name} 
